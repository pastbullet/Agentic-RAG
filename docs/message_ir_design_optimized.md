...
- `sections`：保留局部层次结构；
- `fields`：字段全集；
- `normalized_field_order`：**最终 codegen 的权威字段顺序来源**；
...
第一版不要追求一次覆盖所有消息，建议聚焦 BFD 中最容易打通的认证段：

- `BFD Authentication Section (Simple Password Authentication)`；
- `Keyed MD5 Authentication Section`；
- `Meticulous Keyed MD5 Authentication Section`；
- `Keyed SHA1 Authentication Section`；
- `Meticulous Keyed SHA1 Authentication Section`。
...
2. 能覆盖 BFD 认证段的真实字段布局；
3. 能表达 `Auth Type`、`Auth Len`、`Reserved` 等约束；
4. 能生成真实 `pack/unpack`；
5. 能生成至少一组 roundtrip 测试；
6. codegen 不再依赖模板中的协议特例硬编码；
7. 出现冲突时可产生 diagnostics，而不是静默覆盖；
8. 只有 `normalization_status == ready` 的对象才能进入 codegen。
...
先实现 `MessageIR v1`，只覆盖 BFD 认证段，打通真实 `pack/unpack/validate/test` 流程；在该链路稳定后，再扩展到更复杂的控制报文与组合布局场景。
